...
import { useAnthropometricMeasurementsQuery } from '@/modules/admin/anthropometric/composables/useAnthropometricMeasurementsQuery'
import { useDeleteAnthropometricMeasurementMutation } from '@/modules/admin/anthropometric/composables/useDeleteAnthropometricMeasurementMutation'
...
const params = computed<ListAnthropometricMeasurementsParams>(() => ({
  page: page.value,
  per_page: perPage.value,
  measurement_session_id: searchSessionId.value ? Number(searchSessionId.value) : undefined,
}))

const measurementsQuery = useAnthropometricMeasurementsQuery(params)
const deleteMutation = useDeleteAnthropometricMeasurementMutation()

const measurements = computed<AnthropometricMeasurement[]>(
...
function onEdit(item: AnthropometricMeasurement): void {
  void router.push({ name: 'admin-anthropometric-edit', params: { id: item.id } })
}
function onDelete(item: AnthropometricMeasurement): void {
  const confirmed = window.confirm(`¿Eliminar la medición antropométrica #${item.id}?`)
  if (!confirmed) return
...
<template>
  <section class="space-y-4 md:space-y-5">
    <header class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-stone-800">Mediciones antropométricas</h1>
        <p class="text-sm text-stone-500">
          {{ meta?.total ?? 0 }} registro(s) de estatura, peso y BMI.
        </p>
      </div>
      <RouterLink
        :to="{ name: 'admin-anthropometric-new' }"
      ...
      >
        <i class="ri-scales-2-line" /> Nueva medición
      </RouterLink>
    </header>
        ...
        <table class="w-full text-sm">
          <thead class="bg-stone-50 text-stone-500 uppercase text-[11px] tracking-wide">
            <tr>
              ...
              <th scope="col" class="text-right font-medium py-3 px-4">Estatura</th>
              <th scope="col" class="text-right font-medium py-3 px-4">Peso</th>
              <th scope="col" class="text-right font-medium py-3 px-4">BMI</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-4 align-middle text-stone-600 text-right">{{ m.height_cm }} cm</td>
              <td class="py-3 px-4 align-middle text-stone-600 text-right">{{ m.weight_kg }} kg</td>
              <td class="py-3 px-4 align-middle text-stone-800 text-right font-medium">
                {{ m.bmi }}
              </td>
